                            ...
                            [
                                html.Div(
                                    dcc.Graph(
                                        ...
                                        animate=True,
                                        animation_options={
                                            "frame": {"duration": PCP_TRANSITION_MS, "redraw": True},
                                            "transition": {"duration": PCP_TRANSITION_MS},
                                        },
                                    ),
                                    style={"width": "50%", "display": "inline-block"},
...
def build_dimensions(dff: pd.DataFrame):
    dimensions = []
    for label, col in PCP_DIMS:
        if col not in dff.columns:
            continue
...
def make_hist_figure(dff_all: pd.DataFrame, selected_bin):
    ...
    fig.update_layout(
        template="plotly_white",
        margin=dict(t=10, l=20, r=20, b=30),
        height=240,
        transition={"duration": HIST_TRANSITION_MS},
        uirevision="keep",
...
@app.callback(
    ...
    Input("sat-hist", "clickData"),
    State("selected-sat-bin", "data"),
    prevent_initial_call=True,
)
def toggle_bin(clickData, selected_bin):
...
def update_all(week_range, selected_bin):
    ...
    if dff_all.empty:
        ...
        return [hist_fig] + pcp_figs + [staff_fig]
